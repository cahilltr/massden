...
removeableCols = ["Claims based hospital affiliation CCN 1","Claims based hospital affiliation LBN 1",
                  ...
                  "Organization legal name","Organization DBA name","Group Practice PAC ID","Number of Group Practice members",
...
colsDict = {"NPI":"npi", "Last Name":"last_name", "First Name":"first_name", "Middle Name":"middle_name", "Suffix":"suffix",
            "Gender":"gender", "Credential":"credentials", "Medical school name":"med_school", "Graduation year":"grad_year",
            "Primary specialty":"primary_spec", "Secondary specialty 1":"secondary_spec", "Secondary specialty 2":"tertiary_spec",
            "Secondary specialty 3":"quaternary_spec","Secondary specialty 4":"quinary_spec","All secondary specialties":"all_spec",
...
#createJSON
#For each row in DF, create index and index body and append to body
...
def indexBody(es, df, indexName, type):
    body = createJSONBody(df, indexName,type)
    indexDataBulk(es, body)
...
try:
    ...
    # dataDF = handleCSV(dataFile)
    # body = createJSONBody(dataDF, indexName, typeName)
