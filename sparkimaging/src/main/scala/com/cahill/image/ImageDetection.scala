...
// TODO Use below
    ...
    val lToBOilRatio = 5/6.0
    val lToBContainerRatio = 6/8.0
  //Also test 1 - value for rotations
  //use .5 allowance
    val asdf = cleanedSegmentList
      .filter(cc => {
        val ratio = cc.calculateOrientatedBoundingBoxAspectRatio()
        val oneMinusRation = 1 - ratio
        ~=(ratio, lToBOilRatio, 0.04) || ~=(ratio, lToBContainerRatio, 0.04) || ~=(oneMinusRation, lToBContainerRatio, 0.04) || ~=(oneMinusRation, lToBOilRatio, 0.04)
      })
